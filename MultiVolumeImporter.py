...
class MultiVolumeImporter(ScriptedLoadableModule):
  def __init__(self, parent):
    ...
    parent.index = 0
    parent.helpText = """
    Support of MultiVolume import in Cjyx4
    """
    parent.helpText += self.getDefaultModuleDocumentationLink()
    # MultiVolumeExplorer registers the DRML node type this module is using
...
class MultiVolumeImporterWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    if not NUMPY_AVAILABLE:
      ...
      label = qt.QLabel('You can seek help by contacting 3D Cjyx user list: cjyx-users@bwh.harvard.edu')
      self.layout.addWidget(label)
    ...
    self.__mvSelector = cjyx.qDRMLNodeComboBox()
    self.__mvSelector.nodeTypes = ['vtkDRMLMultiVolumeNode']
    self.__mvSelector.setDRMLScene(cjyx.drmlScene)
    self.__mvSelector.connect('drmlSceneChanged(vtkDRMLScene*)', self.onDRMLSceneChanged)
  ...
  def onDRMLSceneChanged(self, drmlScene):
    self.__mvSelector.setDRMLScene(cjyx.drmlScene)
    return
  ...
  def onImportButtonClicked(self):
    ...
    frameList = []    # frames as DRMLScalarVolumeNode's
    frameFolder = ""
    ...
    mvDisplayNode = cjyx.drmlScene.CreateNodeByClass('vtkDRMLMultiVolumeDisplayNode')
    mvDisplayNode.SetScene(cjyx.drmlScene)
    cjyx.drmlScene.AddNode(mvDisplayNode)
  ...
  def readFrame(self,file):
    ...
    frame = cjyx.vtkDRMLScalarVolumeNode()
  ...
  def annihilateScalarNode(self, node):
    ...
    cjyx.drmlScene.RemoveNode(dn)
    cjyx.drmlScene.RemoveNode(sn)
    cjyx.drmlScene.RemoveNode(node)
