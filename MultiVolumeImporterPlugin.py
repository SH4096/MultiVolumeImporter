...
# from DICOM files into DRML nodes.  It follows the DICOM module's
# plugin architecture.
#
...
class MultiVolumeImporterPluginClass(DICOMPlugin):
  ...
  def settingsPanelEntry(panel, parent):
    ...
    importFormatsComboBox.toolTip = "Preferred format for imported volume sequences. It determines what DRML node type volume sequences will be loaded into."
  ...
  def examine(self,fileLists):
    ...
    if hasattr(cjyx.modules, 'sequences'):
  ...
  def examineFilesMultiseries(self,files):
    ...
    for mvNode in mvNodes:
      ...
      desc = cjyx.dicomDatabase.fileValue(orderedFiles[0],self.tags['studyDescription']) # SeriesDescription
      num = cjyx.dicomDatabase.fileValue(orderedFiles[0],self.tags['seriesNumber'])
      if num != "":
        name = num+": "+desc
      else:
        name = desc
  ...
  def emptyTagValueFound(self,files,tags):
    for f in files:
      for tag in tags:
        value = cjyx.dicomDatabase.fileValue(f,self.tags[tag])
  ...
  def examineFilesIPPInstanceNumber(self,files):
    ...
    desc = cjyx.dicomDatabase.fileValue(files[0],self.tags['seriesDescription']) # SeriesDescription

    minTime = int(cjyx.dicomDatabase.fileValue(files[0],self.tags['instanceNumber']))
    for file in files:
      ipp = cjyx.dicomDatabase.fileValue(file,self.tags['position'])
      time = int(cjyx.dicomDatabase.fileValue(file,self.tags['instanceNumber']))
      if time<minTime:
        minTime = time
    ...
    if nSlicesEqual:
      ...
      for f in range(nFrames):
        ...
        if len(svs)==0:
          ...
          break
        time = float(cjyx.dicomDatabase.fileValue(svs[0].files[0],self.tags['repetitionTime']))*f
        if f==0:
            frameLabelsStr = '0,'
      ...
      mvNode = cjyx.drmlScene.CreateNodeByClass('vtkDRMLMultiVolumeNode')
      mvNode.SetReferenceCount(mvNode.GetReferenceCount()-1)
      mvNode.SetScene(cjyx.drmlScene)
  ...
  def examineFilesIPPAcqTime(self,files):
    ...
    minTime = self.tm2ms(cjyx.dicomDatabase.fileValue(files[0],self.tags['AcquisitionTime']))
    for file in files:
      ipp = cjyx.dicomDatabase.fileValue(file,self.tags['position'])
      time = self.tm2ms(cjyx.dicomDatabase.fileValue(file,self.tags['AcquisitionTime']))
      if time<minTime:
        minTime = time
      if ipp not in subseriesLists:
        subseriesLists[ipp] = {}
    ...
    if nSlicesEqual:
      ...
      for f in range(nFrames):
        ...
        if len(svs)==0:
          print('Failed to parse one of the multivolume frames as scalar volume!')
          break
        time = self.tm2ms(cjyx.dicomDatabase.fileValue(svs[0].files[0],self.tags['AcquisitionTime']))
        if f==0:
            frameLabelsStr = '0,'
            frameLabelsArray.InsertNextValue(0)
            firstFrameTime = time
  ...
  def addAcquisitionAttributes(self,mvNode,frameFileList):
    ...
    for tag in ['EchoTime','RepetitionTime','FlipAngle']:
      if tag != frameTag:
        tagValue = cjyx.dicomDatabase.fileValue(frameFileList[0],self.tags[tag])
  ...
  def examineFiles(self,files):
    ...
    for file in files:
      ...
      value = cjyx.dicomDatabase.fileValue(file,self.tags['seriesInstanceUID']) # SeriesInstanceUID
      desc = cjyx.dicomDatabase.fileValue(file,self.tags['seriesDescription']) # SeriesDescription
  ...
  def isFrameOriginConsistent(self, files, mvNode):
    ...
    for frameNumber in range(nFrames):
      ...
      positionTag = cjyx.dicomDatabase.fileValue(svs[0].files[0], self.tags['position'])
      if positionTag == '':
        return False
  ...
  def load(self,loadable):
    ...
    if loadAsVolumeSequence:
      volumeSequenceNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLSequenceNode",
        cjyx.drmlScene.GenerateUniqueName(baseName))
    ...
    progressbar = cjyx.util.createProgressDialog(labelText="Loading "+baseName,
    ...
    try:
      ...
      for frameNumber in range(nFrames):
        ...
        sNode = cjyx.vtkDRMLVolumeArchetypeStorageNode()
        ...
        if loadAsVolumeSequence:
          ...
          proxyVolume = cjyx.drmlScene.AddNewNodeByClass(frame.GetClassName())
          indexValue = str(frameNumber)
          volumeSequenceNode.SetDataNodeAtValue(proxyVolume, indexValue)
          cjyx.drmlScene.RemoveNode(proxyVolume)
        ...
        if frame.GetDisplayNode():
          cjyx.drmlScene.RemoveNode(frame.GetDisplayNode())
        if frame.GetStorageNode():
          cjyx.drmlScene.RemoveNode(frame.GetStorageNode())
        cjyx.drmlScene.RemoveNode(frame)
      ...
      if loadAsVolumeSequence:
        ...
        # Add browser node
        sequenceBrowserNode = cjyx.drmlScene.AddNewNodeByClass('vtkDRMLSequenceBrowserNode',
          cjyx.drmlScene.GenerateUniqueName(baseName + " browser"))
        ...
        sequencesModule = cjyx.modules.sequences
        ...
        mvDisplayNode = cjyx.drmlScene.AddNewNodeByClass('vtkDRMLMultiVolumeDisplayNode')
  ...
  def initMultiVolumes(self, files, prescribedTags=None):
    ...
    for file in files:
      ...
      for frameTag in consideredTags:
        ...
        tagValueStr = cjyx.dicomDatabase.fileValue(file,self.tags[frameTag])
        if tagValueStr == '':
          # not found?
    ...
    for frameTag in consideredTags:
      ...
      if len(slicesPerFrame) > 1:
        ...
        if self.detailedLogging:
          seriesNumber = cjyx.dicomDatabase.fileValue(file, self.tags['seriesNumber'])
          seriesDescription = cjyx.dicomDatabase.fileValue(file, self.tags['seriesDescription'])
          seriesInstanceUid = cjyx.dicomDatabase.fileValue(file, self.tags['seriesInstanceUID'])
...
class MultiVolumeImporterPlugin:
  ...
  def __init__(self, parent):
    ...
    try:
      ...
      cjyx.modules.dicomPlugins = {}
    cjyx.modules.dicomPlugins['MultiVolumeImporterPlugin'] = MultiVolumeImporterPluginClass
